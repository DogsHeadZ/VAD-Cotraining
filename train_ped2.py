...
def train(config):
    ...
    elif config['generator'] == 'unet2':
        model = UNet(input_channels=12, output_channel=3)
    else:
    ...
    for epoch in range(config['epochs']):
        model.train()
        for j, frames in enumerate(tqdm(train_dataloader, desc='train', leave=False)):
            frames = frames.cuda()
            ...
            pred_flow_input = torch.cat([input_last.unsqueeze(2), outputs.unsqueeze(2)], 2)
            ...
            g_fl_loss = op_loss(flow_gt, flow_pred)

            g_loss = lam_adv * g_adv_loss + lam_gd * g_gd_loss + lam_int * g_int_loss + lam_op * g_fl_loss
