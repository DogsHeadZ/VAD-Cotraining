...
def ObjectLoss_evaluate(test_dataloader, generator, labels_list, videos, dataset, device, frame_height=256, frame_width=256,
    ...
    for k, (frames, objects, bboxes, flow) in enumerate(tqdm(test_dataloader, desc='test', leave=False)):
        ...
        if objects.shape==0:
            ...
            # print("train error: object is none")
            outputs = generator(input, None)
...
if __name__ == "__main__":
    ...
    bbox_folder = "./bboxes/avenue/test"
    flow_folder = "./flow/avenue/test"
    dataset = "avenue"
    ...
    # model_init
    weight_path = "./save/avenue_attention/models/max-frame_auc-model.pth"
    ...
    labels = scipy.io.loadmat('./data/{}_frame_labels.mat'.format(dataset))
     ...
     # 像素级标签
    mask_labels_path = "./data/{}_mask_labels".format(dataset)
    ...
    # test_dataloader, generator, labels_list, videos, dataset, device, frame_height=256, frame_width=256,
    # is_visual=False, mask_labels_path=None, save_path=None, labels_dict=None)
    frame_AUC= ObjectLoss_evaluate(test_dataloader, generator, labels_list, videos, dataset = dataset, device = device,
    is_visual=True, mask_labels_path = mask_labels_path, save_path = save_path, labels_dict=labels) 
    print(frame_AUC)
