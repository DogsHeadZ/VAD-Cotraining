...
from dataset_i3d import Train_TemAug_Dataset_UCF_I3D, Test_Dataset_UCF_I3D
...
def train(config):
    ...
    abnorm_dataset = Train_TemAug_Dataset_UCF_I3D(config['dataset_path'], config['train_split'],
                                                  config['pseudo_labels'], config['clips_num'],
                                                  segment_len=config['segment_len'], type='Abnormal', ten_crop=config['ten_crop'], hard_label=True)
    print(len(norm_dataset))
    print(len(abnorm_dataset))
    ...
    # test
    test_dataset = Test_Dataset_UCF_I3D(config['dataset_path'], config['test_split'],
                                        segment_len=config['segment_len'], ten_crop=config['ten_crop'])
    ...
    for epoch in range(config['epochs']):
        ...
        if epoch % test_epoch == 0:
            ...
            train_utils.log('best_AUC {} at epoch {}, now {}'.format(best_AUC, best_epoch, auc))
