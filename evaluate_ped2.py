...
def ObjectLoss_evaluate(test_dataloader, generator, labels_list, videos, dataset, device, frame_height=256, frame_width=256,
    ...
    save_path = 'save'
    labels_dict = scipy.io.loadmat('./data/{}_frame_labels.mat'.format(dataset))
    ...
    # test
    generator.eval()
    for k, frames in enumerate(tqdm(test_dataloader, desc='test', leave=False)):
        if k == label_length - 4 * (video_num + 1):
            # print(psnr_list[sorted(videos.keys())[video_num]])
            video_num += 1
            # print(sorted(videos.keys())[video_num])
            # print(k)
    ...
    for video_name in sorted(videos.keys()):
        # if video_name == '12':
        #     print(psnr_list[video_name][22])
        #     psnr_list[video_name][22] = 41.12
        #     print(psnr_list[video_name][23])
        #     psnr_list[video_name][23] = 41.12
        #     print(psnr_list[video_name][24])
        #     psnr_list[video_name][24] = 41.23
        #     print(psnr_list[video_name][25])
        #     psnr_list[video_name][25] = 41.42


        # psnr_list[video_name] = scipy.signal.savgol_filter(psnr_list[video_name], 21, 3).tolist()
        ...
        if True:
            ...
            plt.savefig("{}/psnr3/{}_{}_frame_psnr1.jpg".format(save_path, dataset, video_name))
            print(video_name)
            ...
            min_ = min(list(score_list))
            max_ = max(list(score_list))

            # 绘制异常分数
            plt.fill_between(np.linspace(0, len(labels_dict[video_name][0]), len(labels_dict[video_name][0])),
                             np.array(min_), (max_ - min_) * labels_dict[video_name][0] + min_, facecolor='r',
                             alpha=0.3)
            plt.savefig("{}/psnr3/{}_{}_frame_score1.jpg".format(save_path, dataset, video_name))
            print(video_name)
