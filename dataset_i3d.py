...
class Train_TemAug_Dataset_UCF_I3D(Dataset):
    """
    暂时先去掉了pseudo_labels的处理，将hard_label设为True
    """
    def __init__(self, h5_file,train_txt, pseudo_labels,clip_num=8,segment_len=16,
                 type='Normal', ten_crop=False, rgb_diff=False,hard_label=True,score_segment_len=16,continuous_sampling=False):
        self.h5_path = h5_file
        # self.pseudo_labels = np.load(pseudo_labels, allow_pickle=True).tolist()
        self.keys = sorted(list(h5py.File(self.h5_path, 'r').keys()))
        self.clip_num=clip_num
        ...
        if hard_label == False:
            print("hard_label=True are not currently supported")
            exit(1)
        # TODO: hard label = False
    ...
    def get_vid_names_dict(self):
        ...
        for line in open(self.train_txt,'r').readlines():
            ...
            if not "Normal" in key:
                if key in self.video_keys:
                #     # print(key)
                    self.abnorm_vid_names_dict[key] = self.video_keys.count(key)
            else:
                # print(len(key))
                if key in self.video_keys:
                    self.norm_vid_names_dict[key] = self.video_keys.count(key)
                # print(self.norm_vid_names_dict)
        # print(len(self.norm_vid_names_dict))
        # print(len(self.abnorm_vid_names_dict))
...
class Test_Dataset_UCF_I3D(Dataset):
    def __init__(self,h5_file,test_txt,segment_len=16,ten_crop=False,height=256,width=340,crop_size=224):
        self.h5_path = h5_file
        # self.keys = sorted(list(h5py.File(self.h5_path, 'r').keys()))
        self.test_txt=test_txt
        self.segment_len = segment_len
